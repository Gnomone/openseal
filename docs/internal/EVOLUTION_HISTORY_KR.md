...
## 4단계: 검증의 민주화 (Current)
**"검증 도구가 없다면 신뢰는 주장일 뿐이다."**

*   **배경**: 사용자가 `curl`로 받은 결과가 진짜인지 확인할 방법이 없음. "API 문서 보고 직접 계산하세요"는 불친절하고 실수를 유발함.
*   **시도**: `openseal verify` CLI 도구 내장.
*   **철학적 전환**:
    *   **검증 로직 공개**: `g_B` 로직이나 서명 알고리즘을 숨기는 것(Security by Obscurity)보다, **"누구나 검증할 수 있는 도구"**를 배포하여 투명성을 확보하는 것이 진정한 보안임.
    *   **Dev Mode Verify**: 개발 단계에서부터 "소스 수정 -> 검증 실패" 사이클을 체험하게 함으로써 OpenSeal의 가치를 스스로 깨닫게 함.
