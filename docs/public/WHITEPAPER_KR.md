...
2.  **원자적 실행 경계 (Atomic Execution Boundary)**:
    OpenSeal 런타임은 실행 컨텍스트를 독점(Caller Monopoly)합니다. 런타임은 부모 프로세스로서 일회성 실행 토큰(`Nonce`)을 주입하고 감시합니다.

3.  **단방향 봉인 (One-Way Sealing)**:
    실행 완료 즉시, 런타임은 결과를 포착하여 정체성(`A`) 및 Nonce(`N`)와 함께 **동적 비재현 함수**를 통해 결합합니다.

4.  **사후 상태 주장 (Post-State Assertion)**:
    최종 출력물인 `Seal`은 다음을 주장합니다: *"이 결과는 프로젝트 A가 입력 I와 Nonce N을 가지고 실행된 필연적인 결과이다."*
