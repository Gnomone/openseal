...
**출력 결과 예시:**
> ✅ **Root A-Hash**: `19bf5835...` (이 값이 프로젝트의 유일한 정체성입니다)  
...
**결과 확인**: 응답 헤더 또는 JSON에 포함된 서명(`openseal_signature`)을 메모해 두세요. 이것이 바로 **골든 트루스(Golden Truth)**입니다. ✅

### 5단계: 변조 실증 (Tampering Attack)
이제 소스코드를 고의로 수정하여 OpenSeal이 이를 어떻게 감지하는지 확인합니다.

1. `translator.py` 파일을 열어 번역 로직이나 텍스트 처리 방식을 살짝 수정합니다. (예: 특정 단어 추가)
2. 다시 빌드하고 실행합니다:
   ```bash
   openseal build --source . --output ./dist --exec "python3 main.py"
   openseal run --app ./dist --port 7325
...
**결과 확인**: 소스코드가 단 1바이트라도 변조되었다면, 응답의 `signature` 값이 이전과 **완전히 달라졌을 것입니다.** ❌  
이는 OpenSeal이 실행되는 코드의 정체성이 바뀌었음을 암호학적으로 증명한 것입니다.
