# ⚡ OpenSeal 5분 퀵스타트: 내 API 보호하기

이 가이드는 OpenSeal을 사용하여 자신의 API 서비스를 보호하고, 코드 변조 시 이를 어떻게 즉시 감지할 수 있는지 실습하는 과정을 담고 있습니다.

---

## 🚀 단계별 튜토리얼

### 1단계: 샘플 프로젝트 준비 (또는 본인 프로젝트)
테스트를 위해 간단한 API 서버를 준비합니다. 직접 만든 서버가 없다면 [OpenSeal Samples](https://github.com/org/openseal-samples)를 클론하여 시작하세요.

```bash
# 샘플 레포지토리 클론 예시
git clone https://github.com/org/openseal-samples
cd openseal-samples/nodejs-example
npm install
```

> [!NOTE]
> 이 샘플 API는 [설명 준비 중] 입니다. 기본적인 숫차 연산 또는 텍스트 처리 예시를 포함할 예정입니다.

### 2단계: 골든 트루스(Golden Truth) 확보 (Sealing)
프로젝트가 변조되지 않은 "가장 정직한 상태"의 서명을 미리 추출합니다. 이 서명은 나중에 검증의 기준점이 됩니다.

```bash
# openseal-cli를 사용하여 특정 입력에 대한 정답 서명 추출
./openseal seal \
  --app ./nodejs-example \
  --wax "my_first_test_123" \
  --input '{"name": "OpenSeal"}'
```

**출력 결과 예시:**
> ⚖️ **Golden Truth Signature**: `a1b2c3d4e5f6...`  
> (이 값을 메모장에 적어두세요)

### 3단계: OpenSeal 런타임으로 서버 실행
이제 서비스를 OpenSeal 보호막 안에서 실행합니다.

```bash
# 7325 포트로 OpenSeal 런타임 실행 (내부 앱은 자동으로 감지되어 실행됨)
openseal run ./nodejs-example --port 7325
```

### 4단계: 정상 상태 확인
API를 호출하여 응답과 함께 오는 서명이 골든 트루스와 일치하는지 확인합니다.

```bash
curl -X POST http://127.0.0.1:7325/greet \
  -H "X-OpenSeal-Wax: my_first_test_123" \
  -H "Content-Type: application/json" \
  -d '{"name": "OpenSeal"}'
```

**결과 확인**: 응답 JSON의 `openseal.signature` 값이 2단계에서 메모한 값과 일치한다면 **정상**입니다! ✅

### 5단계: 변조 실증 (Tampering Attack)
이제 소스코드를 고의로 수정하여 OpenSeal이 이를 감지하는지 확인합니다.

1. `nodejs-example/index.js` 파일을 열어 응답 텍스트를 살짝 수정합니다. (예: `Hello` -> `Hi`)
2. 서버를 재실행합니다: `openseal run ./nodejs-example --port 7325`
3. 4단계와 **동일한** `curl` 명령어를 다시 실행합니다.

**결과 확인**: 응답의 `signature` 값이 이전과 **완전히 달라졌을 것입니다.** ❌  
이것은 소스코드가 단 1바이트라도 변조되면 결과값에 바인딩된 증명이 깨짐을 의미합니다.

---

## 🛡️ 핵심 정책: Golden Truth
OpenSeal은 실제 프로덕션에서도 동일한 원칙을 적용합니다.

- **은닉**: 서버 로직, 해시 구조 등을 외부에 노출하지 않습니다.
- **증명**: 오직 특정 상황(`Wax` + `Input`)에 대한 **정답 서명(Golden Truth)**만 검증자에게 공유합니다.
- **검증**: 검증자는 응답으로 온 서명이 공유받은 골든 트루스와 일치하는지만 대조하면 됩니다.

---

## 💡 다음 단계
- [보안 메커니즘 상세 보기 (CORE_MECHANISM_KR.md)](./docs/internal/CORE_MECHANISM_KR.md)
- [프로덕션 배포 가이드 (DOCS 가이드 예정)](./docs/production.md)
