...
*   **기준 디렉토리**: 모든 경로는 **프로젝트 루트(빌드 시 소스 경로)**를 기준으로 합니다.
*   **`.gitignore`**: 표준 Git 제외 규칙을 준수합니다.
*   **`.opensealignore`**: OpenSeal 전용 제외 규칙입니다. .gitignore와 동일한 문법을 사용하며, 해당 파일에 정의된 패턴은 **파일 존재 여부부터 내용까지 100% 무시**됩니다.

> [!IMPORTANT]
> **설정 파일의 무결성**: `.opensealignore`, `.openseal_mutable`, 그리고 빌드 시 생성되는 `openseal.json` 자체는 **파일 비수정 검사(A-hash) 대상에 포함**됩니다. 이는 공격자가 제외 규칙을 수정하여 보안을 우회하는 것을 방지하기 위함입니다.
