...
다음 파일들은 **파일 비수정 검사(A-hash)** 대상에서 완전히 제외되며, 빌드 결과물에도 포함되지 않습니다. 프로젝트의 정체성에 영향을 주지 않는 파일들을 정의합니다.
*   **`.gitignore`**: 표준 Git 제외 규칙을 준수합니다. (파일/폴더 경로 기준)
*   **`.opensealignore`**: OpenSeal 전용 제외 규칙입니다. .gitignore와 동일한 문법을 사용하며, 해당 파일에 정의된 경로나 패턴은 **파일 존재 여부부터 내용까지 100% 무시**됩니다.
